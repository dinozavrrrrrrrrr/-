...
<body>
    <div class="avtorizasia">
        <h1>Регистрация</h1>
        <form class="formsub" method="post" enctype="multipart/form-data" action="{{ route('registeration') }}">
            @csrf
            <a href="/"><img class="IMGF" src="img/image 52.png" alt=""></a>
            <label for="title">Имя:</label><br />
            <input placeholder="Введите имя" type="text" class="name" name="name" id="username" required /><br /><br />
            <label for="email">E-mail:</label><br />
            <input placeholder="Введите e-mail" type="email" class="email" name="email" id="email" required /><br /><br />
            <label for="password">Пароль:</label><br />
            <input placeholder="Введите пароль" type="password" class="password" name="password" id="password" required /><br /><br />
            <label for="password">Повторите пароль:</label><br />
            <input placeholder="Введите пароль" type="password" class="password" name="password_confirmation" id="password_repeat"  required /><br /><br />
